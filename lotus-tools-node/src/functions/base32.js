...
const base32 = {
    ...
    encode: function(s) {
        ...
        // Calculate the length of pad by getting the 
...
export function encodeBase32(s) {
    ...
    return trimBase32Padding(encoded);
}
...
// Function to trim padding characters from a Base32 encoded string
export function trimBase32Padding(encodedString) {
    // Find the last non-padding character in the string
    let lastNonPaddingIndex = encodedString.length - 1;
    while (encodedString.charAt(lastNonPaddingIndex) === '=') {
        lastNonPaddingIndex--;
    }
    
    // Return the string up to the last non-padding character
    return encodedString.substring(0, lastNonPaddingIndex + 1);
}
